...
for f in range (ff,lf+1):
	...
	# export view
	ExportView('/home/laset/ray_tracer/a5_'+ str("%03d" % f) + '.pov', view=renderView1)
...
for f in range (ff,lf+1):
	...
	# export view
	ExportView('/home/laset/ray_tracer/a9_'+ str("%03d" % f) + '.pov', view=renderView1)
...
# export view
ExportView('/home/laset/ray_tracer/a5_'+ str("%03d" % frames) + '.pov', view=renderView1)
...
# export view
ExportView('/home/laset/ray_tracer/a9_'+ str("%03d" % frames) + '.pov', view=renderView1)
...
for iso in range (1, 10, 4):


	fin = open( 'a'+ str(iso)+'_'+ str("%03d" % frames) + '.pov', "r" )
...
# write the changed data (list) to a file
	fout = open( 'a'+ str(iso)+'_'+ str("%03d" % frames) + '.pov', "w")
	fout.writelines(data_list)
	fout.close() 

	for line in fileinput.input('a'+ str(iso)+'_'+ str("%03d" % frames) + '.pov', inplace=True):
	    	if line.strip().startswith('mesh2'):
	        	line = '#declare a'+ str(iso)+'_'+ str("%03d" % frames)+' = mesh2 {\n'
...
# Replace the target string
filedata = filedata.replace('a5', 'a5_'+ str("%03d" % frames))

# Replace the target string
filedata = filedata.replace('a9', 'a9_'+ str("%03d" % frames))
...
# Replace the target string
	filedata = filedata.replace('a5', 'a5_'+ str("%03d" % f))

# Replace the target string
	filedata = filedata.replace('a9', 'a9_'+ str("%03d" % f))
...
video = 'ffmpeg -framerate 15 -i p%03d.png -vf format=yuv420p video.mp4'
os.system(video)
